 ...
 CR 
 ." DsspWin : DSSP-приложение со своим окном (работает как TestWin32API ) "
 ...
 [------------------------------------------------------------
  ...
  MSG    msg   [ Сообщение ]           : &msg msg REALADR ;
  WNDCLASSEX  wndclass [ Класс Окна ]  : &wndclass wndclass REALADR ;
  HINSTANCE   hInstance [ Идентификатор (дескриптор) приложения) ]
 
  : !hInstance 	NULL 1 "GetModuleHandleA " WIN32API ! hInstance ;
	 
  : !!!wndclass 0 wndclass WNDCLASSEXlen !!!MB ; [ обнуление всей структуры ]
                   [ Установка отдельных полей : ]
  : !wndclass.cbSize WNDCLASSEXlen  wndclass .cbSize !TW ;
  : !wndclass.style CS_HREDRAW CS_VREDRAW | wndclass .style !TW ;
  : !wndclass.lpfnWndProc FormDsspWndProc wndclass .lpfnWndProc !TL ;
  : !wndclass.hInstance  hInstance wndclass .hInstance !TL ;
  ...
  : !wndclass.hbrBackground WHITE_BRUSH 1 "GetStockObject " WIN32API 
                    wndclass .hbrBackground !TL ;
  : !wndclass.lpszMenuName  NULL wndclass .lpszMenuName !TL ;
  : !wndclass.lpszClassName szAppName wndclass .lpszClassName !TL  ;
...
:: : DsspWin [ ]  [ ON WIN32FAIL! NOP ]
  ...
  [--- ПЕРЕМЕННЫЕ ОКОННОЙ процедуры ---]
     HDC         hdc 
     PAINTSTRUCT ps    : &ps ps REALADR ;
     RECT        rect  : &rect rect REALADR ;
