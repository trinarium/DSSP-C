PROGRAM $DEBUG USE SYSTEM

CR ."Debuger for DSSP-32 on the SPARC"
B16
...
[MON3 - ОБРАБОТКА ОСТАНОВА]
 ...
 : OSTANOV []  OTL1 ."
stoping before "   [OTL1] VYDAT D    1 \M   OTL2 CPC 4+ ZMOTL [] ;
...
: NOINDM ."
 Index ? " .B16 D  CPC 8 +  @L  \B0 [BA,n]
   .", word" .B10 D ." in procedure " [BA] D1 4+ !TL D1 4+ VYDAT D
   EXERR [] ;
...
: V_PROC0 [A в теле] \B0 [BA,n] D ." in procedure "
...
: WRDF []   WRD FIND BR0 ADRS CBA [AZ,AT] C ?INT IF0 NINT [AZ,AT] ;
: ADRS [*] T0 [0]  @ATEK #- =  @ATEK BRD E2D  &0
  IF+ NUMBER [] #* RL = IF0 EXERR TIN1 [0,AT] ;
