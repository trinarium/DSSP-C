...
/***              last modification  20 october 1998   ***/
/*********************************************************/

#define OLD_VCTR /* for old special vector descriptors */
...
//-------------  Get Var address Operation --------------//
 ...
 void _GETADR_OWN (void)	{	//  GET adress of own data
	// cmd points to body of variable 
...
//------- Specifical ARRAY Operations :
 ...
 //////////////////////////////////////////////////////////////
 //////////   SPECIAL  VECTOR TYPE DESCRIPTORS :   //////////
 //////////////////////////////////////////////////////////////
#ifdef	OLD_VCTR

 //--------------  TYPE  Long Int Vector  --------------//
#define	L_SIZE	(4)	// // size of Long 
  cmnd LVctrType [ ] = {  /* body of Descriptor for Long Vector */
	cmd_LCapQ, 	    // for operation CAP?
	cmd_LVctrDec, 	// for operation !1-
	cmd_LVctrInc, 	// for operation !1+
	cmd_T4,     	// for operation SIZE?
	cmd_VDimQ,  	// for operation DIM?
	cmd_LVctrSub, 	// for operation !-
	cmd_LVctrAdd, 	// for operation !+
	cmd_LVctrSt0, 	// for operation !0
	cmd_LVctrSt1, 	// for operation !1
	cmd_LVctrAllSet,// for operation !!!
	cmd_LVctrAdr,	/* for operation '   */
	cmd_LVctrSt,	// for operation !
	cmd_LVctrVal    // for operation on Name
  };
 //------- Specifical Operations :
 void _LVctrAdr (void)	{	// Push Address of Vctr[i] in A-stack
	// AITEM(1) = index; AT= base address of Vector
	_TstVctrInd();
	AITEM(1)=AT+AITEM(1)*L_SIZE; 
	ADEL;
	// AT= address of item Vector[index]
 }
 void _LVctrVal (void)	{	// Get Long Vctr[i] value 
	 _LVctrAdr();
	 _LVarVal();
 }
 void _LVctrSt (void)	{	// Set (store) Long Vctr[i] value
	 _LVctrAdr();
	 _LVarSt();
 }
 void _LVctrAllSet (void) {	// Assign value to every item Vctr[i] 
	 APUSH(ML(DBAdr+V_COUNT));
	 // A-Astack contains now: [Value, Address, Number of items ]
	 _SetL(); // execute !!!ML
 }
 void _LVctrSt1 (void)	{	// Assign value 1 to Vctr[i]
	 _LVctrAdr();
	 _LVarSt1();
 }
 void _LVctrSt0 (void)	{	// Assign value 0 to Vctr[i]
	 _LVctrAdr();
	 _LVarSt0();
 }
 void _LVctrAdd (void)	{	// Add value to Vctr[i]
	 _LVctrAdr();
	 _LVarAdd();
 }
 void _LVctrSub (void)	{	// Sub value from Vctr[i]
	 _LVctrAdr();
	 _LVarSub();
 }
 void _LVctrInc (void)	{	// Add 1 to Vctr[i]
	 _LVctrAdr();
	 _LVarInc();
 }
 void _LVctrDec (void)	{	// Sub 1 from Vctr[i]
	 _LVctrAdr();
	 _LVarDec();
 }
 void _LCapQ (void)	{	// size of whole Vector
	 _VDimQ();	// AT = number of items in Vector
	 AT *= L_SIZE;
	 // AT = // size of whole Vector (in bytes)
 }

  //--------------  TYPE  Act Vector  --------------//
  cmnd AVctrType [ ] = {  /* body of Descriptor for Act Vector */
	cmd_LCapQ, 	    // for operation CAP?
	cmd_LVctrDec, 	// for operation !1-
	cmd_LVctrInc, 	// for operation !1+
	cmd_T4,     	// for operation SIZE?
	...
	cmd_LVctrAdr,	/* for operation '  */
	cmd_LVctrSt,	// for operation !
	cmd_AVctrVal    // for operation on Name (execute)
  };
 //------- Specifical Operations :
 void _AVctrVal (void)	{	// // Execute Procedure of Act Vctr[i]
	 _LVctrAdr();
	 _AVarVal();
 }

 //--------------  TYPE  Word Vector  --------------//
#define	W_SIZE	(2)	// // size of Word
  cmnd WVctrType [ ] = { /* body of Descriptor for Word Vector */
	cmd_WCapQ, 	    // for operation CAP?
	cmd_WVctrDec, 	// for operation !1-
	cmd_WVctrInc, 	// for operation !1+
	cmd_T2,     	// for operation SIZE?
	cmd_VDimQ,  	// for operation DIM?
	cmd_WVctrSub, 	// for operation !-
	cmd_WVctrAdd, 	// for operation !+
	cmd_WVctrSt0, 	// for operation !0
	cmd_WVctrSt1, 	// for operation !1
	cmd_WVctrAllSet,// for operation !!!
	cmd_WVctrAdr,	/* for operation '  */
	cmd_WVctrSt,	// for operation !
	cmd_WVctrVal    // for operation on Name
  };
 //------- Specifical Operations :
 void _WVctrAdr (void)	{	// Push Address of Vctr[i] in A-stack
	// AITEM(1) = index; AT= base address of Vector
	_TstVctrInd();
	AITEM(1)=AT+AITEM(1)*W_SIZE; 
	ADEL;
	// AT= address of item Vector[index]
 }
 void _WVctrVal (void)	{	// Get Word Vctr[i] value 
	 _WVctrAdr();
	 _WVarVal();
 }
 void _WVctrSt (void)	{	// Set (store) Word Vctr[i] value
	 _WVctrAdr();
	 _WVarSt();
 }
 void _WVctrAllSet (void) {	// Assign value to every item Vctr[i] 
	 APUSH(ML(DBAdr+V_COUNT));
	 // A-Astack contains now: [Value, Address, Number of items ]
	 _SetW(); // execute !!!MW
 }
 void _WVctrSt1 (void)	{	// Assign value 1 to Vctr[i]
	 _WVctrAdr();
	 _WVarSt1();
 }
 void _WVctrSt0 (void)	{	// Assign value 0 to Vctr[i]
	 _WVctrAdr();
	 _WVarSt0();
 }
 void _WVctrAdd (void)	{	// Add value to Vctr[i]
	 _WVctrAdr();
	 _WVarAdd();
 }
 void _WVctrSub (void)	{	// Sub value from Vctr[i]
	 _WVctrAdr();
	 _WVarSub();
 }
 void _WVctrInc (void)	{	// Add 1 to Vctr[i]
	 _WVctrAdr();
	 _WVarInc();
 }
 void _WVctrDec (void)	{	// Sub 1 from Vctr[i]
	 _WVctrAdr();
	 _WVarDec();
 }
 void _WCapQ (void)	{	// size of whole Word Vector
	 _VDimQ();	// AT = number of items in Vector
	 AT *= W_SIZE;
	 // AT = // size of whole Vector (in bytes)
 }
...
//--------------  TYPE Byte Vector  --------------//
  cmnd BVctrType [ ] = { /* body of Descriptor for Byte Vector */
	cmd_VDimQ, 	    // for operation CAP? like for DIM?
	cmd_BVctrDec, 	// for operation !1-
	cmd_BVctrInc, 	// for operation !1+
	cmd_T1,     	// for operation SIZE?
	cmd_VDimQ,  	// for operation DIM?
	cmd_BVctrSub, 	// for operation !-
	cmd_BVctrAdd, 	// for operation !+
	cmd_BVctrSt0, 	// for operation !0
	cmd_BVctrSt1, 	// for operation !1
	cmd_BVctrAllSet,// for operation !!!
	cmd_BVctrAdr,	/* for operation '  */
	cmd_BVctrSt,	// for operation !
	cmd_BVctrVal    // for operation on Name
  };
 //------- Specifical Operations :
 void _BVctrAdr (void)	{	// Push Address of Vctr[i] in A-stack
	// AITEM(1) = index; AT= base address of Vector
	_TstVctrInd();
	AITEM(1)=AT+AITEM(1); 
	ADEL;
 ...
 void _BVctrVal (void)	{	// Get Byte Vctr[i] value 
	 _BVctrAdr();
	 _BVarVal();
 }
 void _BVctrSt (void)	{	// Set (store) Byte Vctr[i] value
	 _BVctrAdr();
	 _BVarSt();
 }
 void _BVctrAllSet (void) {	// Assign value to every item Vctr[i] 
	 APUSH(ML(DBAdr+V_COUNT));
	 // A-Astack contains now: [Value, Address, Number of items ]
	 _SetB(); // execute !!!MB
 }
 void _BVctrSt1 (void)	{	// Assign value 1 to Vctr[i]
	 _BVctrAdr();
	 _BVarSt1();
 }
 void _BVctrSt0 (void)	{	// Assign value 0 to Vctr[i]
	 _BVctrAdr();
	 _BVarSt0();
 }
 void _BVctrAdd (void)	{	// Add value to Vctr[i]
	 _BVctrAdr();
	 _BVarAdd();
 }
 void _BVctrSub (void)	{	// Sub value from Vctr[i]
	 _BVctrAdr();
	 _BVarSub();
 }
 void _BVctrInc (void)	{	// Add 1 to Vctr[i]
	 _BVctrAdr();
	 _BVarInc();
 }
 void _BVctrDec (void)	{	// Sub 1 from Vctr[i]
	 _BVctrAdr();
	 _BVarDec();
 }
...
void sSSsbi(void)
{
  ...
  len=ML(dest);
  delta=ATPOP;
  dest=dest+(int32)DsspMem+4+delta;
  lend=ML(DBAdr+V_COUNT);
  lens=ATPOP;
  source=ATPOP+(int32)DsspMem;
  sEnd=*(char*)(source+lens); *(char*)(source+lens)=0;
  ...
  *(char*)(dest+len)=dEnd;
  *(char*)(source+lens)=sEnd;
  AINS; AT=(int32)result-dest+delta;
